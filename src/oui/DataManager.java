...
public class DataManager {
	...
	public static void CreateIndex(String columnName, IndexPanel ip) {
		...
		rowsTotal = Integer.parseInt(metadataContent); //defining the total number of rows
		//Parsing each row from data pages and adding that row in the index pages/Btree
		while (rowsCounter <= rowsTotal) {
			...
			try {
				File file = new File(pagePath); // created object of file
				BufferedReader reader = new BufferedReader(new FileReader(file)); // passed the file to filereadrer
																			      // created a BufferedReader of type filereader
				int offset = 0;
				...
				do {
					...
					if (line != null && line.trim().length() > 0) {
						System.out.println(rowsCounter + ". " + line); // reading each line from data
						String value = line.split("[|]")[columnIndex]; // fetching the value of the index attribute
	...
	private static void addLineToNode(String indexDirPath, String node, HashMap<String, Boolean> nodeLeafMap,
		...
		if (!isLeaf) {
			...
			manageChildNodeSize(indexDirPath, childNode, node, nodeLeafMap); // managing child node ka size
		}
		 else {
			// adding to leaf node
			// not taking care of splitting at this moment
			// will consider splitting when will reach to parent node
			try {
				File file = new File(indexDirPath + "/" + node);
				BufferedReader reader = new BufferedReader(new FileReader(file));
		...
		if (fileSize > 8 * 1024) {
			...
			try {
				...
				if (childNode == "root") {
					...
					// root got splitted into child and sibling node
				...
				// node should have pointer to child node and sibling node
				String lineTI = lines.get(lines.size() / 2); //mid value
				lines.clear();
	...
	private static void searchValueInIndex(String indexDirPath, String node,
		...
		if (!isLeaf) {
			...
			// Means the given node is leaf node
